...
class CRBrowser extends browser_1.BrowserBase {
    ...
    static async connect(transport, options, devtools) {
        const connection = new crConnection_1.CRConnection(transport_1.SlowMoTransport.wrap(transport, options.slowMo), options.loggers);
    ...
    _onAttachedToTarget({ targetInfo, sessionId, waitingForDebugger }) {
        ...
        if (targetInfo.type === 'background_page') {
            ...
            backgroundPage.pageOrError().then(() => {
                context.emit(events_2.Events.ChromiumBrowserContext.BackgroundPage, backgroundPage._page);
            });
            return;
        }
        ...
        if (targetInfo.type === 'service_worker') {
            ...
            context.emit(events_2.Events.ChromiumBrowserContext.ServiceWorker, serviceWorker);
            return;
        }
...
class CRBrowserContext extends browserContext_1.BrowserContextBase {
    ...
    async _doClose() {
        helper_1.assert(this._browserContextId);
