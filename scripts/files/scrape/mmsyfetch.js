...
await page.goto('http://quran.mmsy.info/app/',{timoeout:160000})
await page.check('input[name="rdo_trans"][value="my.htm"]');
for(i=1;i<=114;i++){

await page.selectOption('select#lst_sura', i+'');
await new Promise(r => setTimeout(r, 7000));

textarr = await page.evaluate(() => {
  names = Array.from(document.querySelectorAll('.trans'))
     return names.map(e=>e.textContent)
});


fs.appendFileSync('burma1.txt',textarr.join('\n'))
