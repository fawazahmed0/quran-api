...
for filename,pageno in zip(dirlist, range(1,len(dirlist)+1)):
    ...
    #filename = 'mandarunsec_Page_003.tiff'
    ...
    if lines is not None:
        ...
        simplelines = lines[:, 0]
        firstVLine = getVLineVal(simplelines,50,10000,350,620, False)
        if firstVLine is not None:
            xcord = firstVLine[2] + padding
        secondVLine = getVLineVal(simplelines,50,10000,650,870, False)
        if secondVLine is not None:
            width = secondVLine[0] - padding
